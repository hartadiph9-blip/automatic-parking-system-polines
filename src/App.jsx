    ...
    <div className="bg-slate-900 text-slate-100 font-sans min-h-screen">
      <header className="bg-slate-800 border-b border-slate-700 p-6 shadow-md flex justify-between items-center">
        ...
        <div className="flex gap-4 items-center">
          ...
          <button onClick={() => window.location.hash = ''} className="text-sm bg-slate-700 hover:bg-slate-600 px-3 py-2 rounded transition-colors">Kembali ke Menu</button>
        </div>
      </header>
...
function PublicWeb() {
  ...
  useEffect(() => {
    // 1. Ambil data awal untuk menghitung slot yang sedang terpakai
    const fetchSlots = async () => {
      const { data } = await supabase.from('parking_logs').select('id, rfid_id, status').eq('status', 'IN');
      if (data) setLogs(data);
    };
    fetchSlots();

    // 2. Listen Realtime untuk Update Layar
    const channel = supabase.channel('public-channel')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'parking_logs' }, async (payload) => {
        if (payload.new.status === 'IN') {
          setLogs(prev => [...prev, payload.new]); // Kurangi slot
          triggerNotification(payload.new.rfid_id, 'IN'); // Tampilkan Popup
        }
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'parking_logs' }, async (payload) => {
        if (payload.new.status === 'OUT') {
          setLogs(prev => prev.filter(log => log.id !== payload.new.id)); // Tambah slot
          triggerNotification(payload.new.rfid_id, 'OUT'); // Tampilkan Popup
        }
      })
      .subscribe();
  ...
  const triggerNotification = async (rfid, type) => {
    // Cari data nama & plat di tabel members
  ...
  return (
    <div className="bg-black text-white font-sans h-screen flex flex-col relative overflow-hidden">
      {/* Tombol kembali (tersembunyi di pojok kiri atas untuk admin) */}
      ...
      {/* Tampilan Standar (IDLE) - Menunjukkan Slot Parkir */}
      <div className={`absolute inset-0 flex flex-col items-center justify-center transition-opacity duration-700 ${notify.show ? 'opacity-0' : 'opacity-100'}`}>
        <h1 className="text-4xl md:text-5xl font-bold text-slate-300 tracking-widest mb-12 text-center">
          SISTEM PARKIR CERDAS<br/><span className="text-yellow-400">POLITEKNIK NEGERI SEMARANG</span>
        </h1>
        
        <div className={`border-4 rounded-3xl p-12 text-center w-3/4 max-w-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] ${isFull ? 'border-red-600 bg-red-950/30' : 'border-blue-600 bg-slate-900/50'}`}>
          <h2 className="text-4xl text-slate-400 mb-6 font-semibold uppercase">Informasi Ketersediaan Slot</h2>
          
          {isFull ? (
            <div className="animate-pulse">
              <span className="text-8xl font-black text-red-500 tracking-tighter">PENUH</span>
              <p className="text-3xl text-red-300 mt-6">Mohon tunggu kendaraan keluar.</p>
            </div>
          ) : (
            <div>
              <span className="text-9xl font-black text-green-400 tracking-tighter">{availableSlots}</span>
              <span className="text-6xl text-slate-500 font-medium"> / {MAX_SLOTS}</span>
              <p className="text-3xl text-blue-300 mt-6 animate-pulse">Sistem Auto-Gate Aktif. Silakan Tap Kartu.</p>
            </div>
          )}
        </div>
      </div>
...
export default function App() {
  ...
  // Deteksi URL Hash saat pertama kali dimuat & setiap kali ada perubahan
  useEffect(() => {
    const handleHash = () => setCurrentView(window.location.hash);
    handleHash(); // Cek saat render pertama
  ...
  // Halaman Landing (Jika tidak ada akhiran di URL)
  return (
    <div className="bg-slate-900 min-h-screen flex flex-col items-center justify-center font-sans text-white p-4">
      ...
      <div className="flex flex-col md:flex-row gap-6 w-full max-w-4xl justify-center">
        ...
        <button 
          ...
          className="bg-slate-800 hover:bg-slate-700 border border-blue-900/50 hover:border-blue-500 p-8 rounded-2xl flex flex-col items-center text-center transition-all group flex-1"
        >
          ...
          <p className="text-sm text-slate-400">Dasbor untuk pendaftaran kartu RFID dan pemantauan jumlah kendaraan secara Live.</p>
        </button>

        {/* Tombol Web Pengguna */}
        <button 
          onClick={() => window.location.hash = 'public'}
          className="bg-slate-800 hover:bg-slate-700 border border-green-900/50 hover:border-green-500 p-8 rounded-2xl flex flex-col items-center text-center transition-all group flex-1"
        >
          ...
          <p className="text-sm text-slate-400">Tampilan VMS untuk diletakkan di gerbang parkir. Menampilkan sisa slot dan ucapan otomatis.</p>
        </button>
      </div>
    </div>
  );
